...
export async function getCountriesWithStartingPrice(): Promise<
  ApiResponse<Country[]>
> {
  try {
    const response =
      await api<CountriesThatHavePackagesResponeType>("/packages/country");
...
export async function getDataOnlyPackages(
  countrySlug: string,
): Promise<ApiResponse<PackagesData>> {
  if (countrySlug === "error-demo")
    throw new Error("Intentional error for the error.tsx demonstration.");
  await pause(900);
  try {
    ...
    return {
      status: false,
      message:
        error instanceof Error ? error.message : "Could not load packages",
    };
  }
}
